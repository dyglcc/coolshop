...
public class DetailActivity extends BaseActivity implements View.OnClickListener, ObserverScrollView.ScrollViewListener {
    ...
    private void initData() {
        ...
        @SuppressLint("StaticFieldLeak") AsyncTask task = new AsyncTask() {
            ...
            protected Object doInBackground(Object[] objects) {
                ...
                Request request = new Request.Builder().url(AppConstant.BASE_URL + mGoodsId + ".txt").build();
                try {
    ...
    public void onSucceed(Response response) {
        ...
        Gson gson = new Gson();
        try {
            mDetailInfo = gson.fromJson(response.body().string(), GoodsDetailInfo.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
        //商品名称
    ...
    public void onScroll(ObserverScrollView scrollView, int x, int y, int oldX, int oldY) {
        if (y <= 0) {
            ...
            mTitleTvTitle.setVisibility(View.VISIBLE);
            if (mDetailInfo != null && mDetailInfo.getResult() != null && mDetailInfo.getResult().getProduct() != null) {
